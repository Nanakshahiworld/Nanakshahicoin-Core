...
const char * const BITCOIN_CONF_FILENAME = "nanakshahicoin.conf";
const char * const BITCOIN_PID_FILENAME = "nanakshahicoin.pid";
...
#else
    const char* pszModule = "nanakshahicoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nanakshahicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Nanakshahicoin";
#else
    // Unix
    return pathRet / ".nanakshahicoin";
#endif
#endif
}
